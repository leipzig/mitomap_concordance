...
gball %>% mutate(syn=str_extract(cal_aachange,'((non-)?syn)')) %>% filter(!is.na(syn)) %>% group_by(syn) %>% summarize(total_events=n()) -> kaks
kable(kaks)
```
This ratio is `r kaks$total_events[1]/kaks$total_events[2]`

By gene
```{r kaksbygene, echo=FALSE, message=FALSE}
gball %>% mutate(syn=str_extract(cal_aachange,'((non-)?syn)')) %>% filter(!is.na(syn)) %>% select(tpos,syn) -> aachange
names(aachange)<-c("pos","change")
