...
rs <- dbSendQuery(con,"select genbank_id, tpos, tnt, qnt, haplogroup, ismmut from mitomap.genbank")
...
sample_subset<-levels(gbfac$genbank_id)[sample(1:length(levels(gbfac$genbank_id)),5000)]

PCA<-snpgdsPCA(genofile,autosome.only=FALSE,num.thread=16,eigen.cnt=2,sample.id = sample_subset,snp.id = snp_subset)

gbhap$africa_only<-apply(gbhap,1,function(x){if(x['region']=='Africa'){return(x['prefix'])}else{return(x['region'])}})


tab <- data.frame(sample.id = PCA$sample.id,
                  pop = gbhap[match(PCA$sample.id,gbhap$genbank_id),"region"],
                  pre = gbhap[match(PCA$sample.id,gbhap$genbank_id),"prefix"],
                  afronly = gbhap[match(PCA$sample.id,gbhap$genbank_id),"africa_only"],
    EV1 = PCA$eigenvect[,1],    # the first eigenvector
    EV2 = PCA$eigenvect[,2],    # the second eigenvector
    stringsAsFactors = FALSE)
ggplot(tab,aes(EV1,EV2))+geom_point(aes(color=region))
```
The regional PCA is interesting. If we provide a breakdown just by Africa:
```{r, echo=FALSE}
ggplot(tab,aes(EV1,EV2))+geom_point(aes(color=afronly))
```
This makes more sense if we view
