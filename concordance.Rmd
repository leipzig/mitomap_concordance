...
group_by(genbank_count, locus, type) %>% mutate(length=get_eligible_length(locus)) %>% summarise(total_variants=sum(cnt),rate=round(sum(cnt)/mean(length),length=mean(length),digits=2)) -> loci_conservation_total_weight
...
privhapdf<-data.frame(Haplogroup=names(privatehap),Private=privatehap,row.names=NULL)
names(privdf)<-"perc private"
datatable(privhapdf,rownames=FALSE)
```
