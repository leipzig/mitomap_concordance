...
```{r setup, echo=TRUE, message=FALSE, cache=TRUE, fig.path='{{SLINK}}/'}
library(ggplot2)
library(dplyr)
library(RPostgreSQL)
library(stringr)
...
```{r dnsnp, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
There are `r length(unique(gball$snpid))` distinct mutations in the gb set over `length(unique(gball$tpos))` positions.

```{r varfreq, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r common_variants, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
datatable(by_var_cnt[order(-by_var_cnt$variants),][1:30,])
```

# Most and least conserved loci (over 20 bp, beware overlapping annotation)
```{r loci, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r least_conserved, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
datatable(subset(loci_conservation[order(-loci_conservation$rate),],length>20)[1:10,])
```

## Most Conserved Loci (over 20bp)
```{r most_conserved, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
datatable(subset(loci_conservation[order(loci_conservation$rate),],length>20)[1:10,])
```

## Variation rate by type
Multiple regressions are evident by locus type
```{r regressors, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r titvg, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r titvc, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r kaks, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
By gene
```{r kaksbygene, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
datatable(table(annoaa$locus,annoaa$change)[rowSums(table(annoaa$locus,annoaa$change))>30,])
```

# Population genetics
```{r popgen, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r byreg, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
datatable(as.data.frame(table(gbhap$region)))
...
```{r bypre, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
datatable(as.data.frame(table(paste(gbhap$region,gbhap$prefix))))
...
## All regions
```{r pca, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r afrbreak, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r doregion, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
# Europe only
```{r eurosetup, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
if(MASTER_SAMPLE_NUMBER){
...
```{r europe, ref.label='doregion', echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
```

# Asia only
```{r asiassetup, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
if(MASTER_SAMPLE_NUMBER){
...
```{r asia, ref.label='doregion', echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
```


## Which regions have the most private mutations? i.e. how many haplogroups fall slightly outside of Phylotree?
```{r, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
datatable(prop.table(table(str_detect(gbhap$haplogroup_verbose,'\\+'),gbhap$prefix),2)*100)
```
...
```{r disease_reg, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
gball$disease_anno<-ifelse(gball$disease=='','',paste(gball$disease,gball$snpid))
datatable(round(prop.table(table(gball$disease_anno[gball$disease_anno!=''],gball$region[gball$disease_anno!='']))*100,digits=3))
```

## Which diseases segregate between Afr and Eur+Asia
```{r disease_fish, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
adj_afr_disease_pvals_below_thresh<-as.data.frame(sort(adj_afr_disease_pvals[adj_afr_disease_pvals<0.01]))

datatable(adj_afr_disease_pvals_below_thresh)
```
...
```{r market_basket, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
...
```{r conservation, echo=TRUE, message=FALSE, cache=TRUE, dependson="setup", fig.path='{{SLINK}}/'}
