...
genbank_count[Reduce("|",get_region_logical_lists(asia)),"Region"]<-"Asia"
genbank_count[Reduce("|",get_region_logical_lists(africa)),"Region"]<-"Africa"
genbank_count[Reduce("|",get_region_logical_lists(europe)),"Region"]<-"Europe"
```
...
get_region_logical_lists_gb_hap<-function(regionprefixes){
    lapply(regionprefixes,function(a_haplogroup_prefix){
      str_detect(gbhap$haplogroup,paste('^',a_haplogroup_prefix,sep=""))
  })
}
gbhap$Region<-NA
gbhap[Reduce("|",get_region_logical_lists_gb_hap(asia)),"Region"]<-"Asia"
gbhap[Reduce("|",get_region_logical_lists_gb_hap(africa)),"Region"]<-"Africa"
gbhap[Reduce("|",get_region_logical_lists_gb_hap(europe)),"Region"]<-"Europe"
dbDisconnect(con)
```
The breakdown of GenBank sequences by region
```{r, echo=FALSE}
kable(table(gbhap$Region))
```
Which regions have the most private mutations?
```{r, echo=FALSE}
...
snp_subset<-levels(gbfac$snpid)[1:2000]
sample_subset<-levels(gbfac$genbank_id)[1:2000]
PCA<-snpgdsPCA(genofile,autosome.only=FALSE,num.thread=16,missing.rate=.1,eigen.cnt=2,sample.id = sample_subset,snp.id = snp_subset)
